...
function [errNorm01] = FluxRecons(FRsch, P_in, h_in, alpArr, dt, tf, efreq, plotFreq)
%
% Input:
...
kN = k0;%(P+1)*pi;                                                              % Nyquist frequency for P=P_in, h=1
...
% Update
%figure(100*P_in + floor(10000*h_in) + 20)
%figure
while 1
...
%         plotFR2p(1,'DG',t(iter),uex(xex,t(iter)),1);
    ...
    u = ui + (dt/6)*(Fp0 + 2*Fp1 + 2*Fp2 + Fp3);
       ...
       % Check for passage of a period
       %t(iter)
    if (abs(t(iter) - (xbdry(2) - xbdry(1))  ) <=0.9*dt)
        disp('A period has passed!')
        uref = u;
        upref = D*u;
    end
    ...
    if plotFlag
         plot(reshape(x,[1 numel(x)]),reshape(u,[1 numel(u)]),'b-^')
     ylim([-1.1 1.1])
     drawnow
    end
    % Check for completion
    if (t(iter)-tf > 1e-10); break; end
    
end

%% Truncate zero entries

[errNorm01(1), errNorm01(2)] = errNorm(u,uref);
%cnt = 1;
